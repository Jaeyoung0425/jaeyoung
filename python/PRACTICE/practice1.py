...
class SGNS(torch.nn.Module):
    def __init__(self, D_dim, W_dim): #d_dim = 단어개수,  w_dim = 차원
        super(SGNS, self).__init__()
        self.dict_len = D_dim #D_dim 
        # first embedding layer, for center word
        self.linear1 = torch.nn.Linear(D_dim, W_dim) #D_dim, W_dim 
        # second embedding layer, for neighbor word  
        self.linear2 = torch.nn.Linear(D_dim, W_dim) #D_dim, W_dim
    
    #forward = 예측을 수행하는 함수
    # number_1 is the number of center vocab, number_2 is the number of neighbor vocab
    def forward(self, number_1, number_2):
        try:
            # create one-hot vector for the center word 중심 단어의 원핫벡터
            temp_tensor = torch.Tensor([0] * self.dict_len) # self.dict_len #temp_tensor는 단어 길이만큼 0인 벡터가 됨
            temp_tensor[number_1] = 1 # 1 
            
            # create projection layer 
            projection = self.linear1(temp_tensor) #projection layer = 활성화 함수가 존재하지 않고 단순히 행렬곱을 통한 선형 변환
            
            #선형변환 = 벡터의 성분값을 바꾸는 것이 아니라 벡터 공간의 기저 자체를 변환함으로써 새로운 벡터 공간을 만드는 작업
            #결국 projection도 벡터인것
            
            # create one-hot vector for the neighbor word 주변 단어의 원핫벡터 
            temp_tensor2 = torch.Tensor([0] * self.dict_len) 
            temp_tensor2[number_2] = 1
            
            # create projection layer for neighbor word 
            projection2 = self.linear2(temp_tensor2) #
            
            # inner product two word
            output = torch.dot(projection, projection2) #projection, projection2 .dot(a,b) = 두 벡터의 내적(내적은 행렬곱이 아님)
            #output = [n] 이런 모양
            
            # activate using sigmoid function
            output = torch.nn.sigmoid(output) 
            # unsqueeze output
            output = output.unsqueeze(0) # unsqueeze를 해서 [[n]] 이런 모양이 됨 (1x1)
            
            # add the opposite possibility ( if output = 0.7, append 0.3 ) 
            # 주변 단어가 아닐 확률도 output tensor에 더해주기 -> 크로스 엔트로피 에러를 구할 때 필요하기 때문
            output = torch.cat((torch.Tensor([1 - output[0].item()]), output), dim=-1) # dim = -1 마지막 차원에 대해서 더하기
            #output[0] -> 0 = 위치
            #cat함수 = concatenate하고자 하는 차원을 증가시킴, 두 텐서의 차원을 더한 값으로 차원이 변경됨
            return output
...
class SGNS(torch.nn.Module):
    ...
    def forward(self, dataset, desc):
        ...
        try:
            x_temp, y_temp, z_temp = dataset
            if desc is True: #neighbour relation
                result = torch.empty(0)
                for i in range(len(x_temp)): #len(x_temp) = the number of the center words
...
for title in tqdm(title_list):
    for word_num in range(len(title)): #word_num = 숫자
        temp_list = []
        for _ in [-2,-1,1,2]:
            if (word_num + _ < 0) or (word_num + _ >= len(title)): #word_num보다 len(title)이 하나 더 많음 / 이게 다 0부터 세서 그럼
                pass #문장의 길이보다 작거나 넘치면 그에 해당하는 단어를 찾을 수 없음
            else:
                temp_list.append(title[word_num + _]) #중심단어 하나에 해당하는 주변단어 
        
        center_list.append(title[word_num])
        neighbor_list.append(temp_list)
        
        rand = [randiant(0,sp.GetPieceSize()-1) for i in range(6)]
        rand = set(rand)
        
        rand = rand-set(temp_list) #주변단어를 제외한 진짜 랜덤한 단어
        rand = list(rand)
        rand_list.append(rand)
        #여기까지 center , neighbour, random list 만들기 완료
        
#batch 만들기        
neighbor_list = [[-1] * (4 - len(batch)) + batch for batch in neighbor_list]
rand_list = [[-1] * ( 6 - len(batch)) + batch for batch in rand_list]

x_trian = torch.LongTensor(center_list)
...
#Dataset = Dataset을 상속한 클래스, 학습데이터와 레이블을 묶어놓은 것 
#DataLoader = batch 관리 담당, batch의 반복을 편리하게 해줌, 미니배치를 자동으로 제공함

for batch_idx, samples in tqdm(enumerate(dataloader)): #samples = dataloader 안에 들어있는 애들
    result = model.forward(samples, True)
    loss = criterion(result, torch.LongTensor([1] * len(result)))
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()    
    
    result = model.forward(samples, False)
    loss = criterion(result, torch.LongTensor([0] * len(result)))
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()    
    ...
    def forward(self, dataset, desc):
        try:
                ...
                for i in range(len(x_temp)): 
                    for j in range(len(y_temp[i])):
                        if y_temp[i][j].item == -1:
                            ...
                            projection2 = self.embedding2(torch.tensor([y_temp[i][j].item()])) 
            ...
            if desc is False:
                result = torch.empty(0)
                for i in range(len(x_temp)):
                    for j in range(len(z_temp[i][j])):
                        if z_temp[i][j].item() == -1:
                            pass
                        else:
                            projection = self.embedding1(torch.tensor([x_temp[i].item()]))
                            projection2 = 
